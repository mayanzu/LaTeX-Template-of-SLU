% ================= 文档用途说明 =================
%{
%   用途：本模板为安徽三联学院本科毕业论文通用模板
%   版本：v1.2.2-comment-style (2025-05-20)
%   注意事项：
%     1. 使用XeLaTeX编译
%     2. 封面存放于./cover目录
%     3. 摘要存放于./abstract目录
%     4. 不同章节分不同文件，存放于./body目录
%     4. 致谢存放于./acknowledge目录
%     5. 图片存放于./pic目录
%     6. 文献引用使用thebibliography直接管理
%     7. %需要使用转义符\%表示
%}
...
\usepackage{listings,lstautogobble} % 代码块排版与自动去除前导空白
...
\newcommand{\upcite}[1]{$^{[#1]}$} % 自定义上标引用命令 \upcite{bibkey}
...
\definecolor{codegreen}{rgb}{0,0.6,0}       % 绿色 (可用于自定义，当前 listings 未直接使用此名称的颜色)
\definecolor{codegray}{rgb}{0.5,0.5,0.5}        % 灰色 (用于 listings 行号)
\definecolor{codepurple}{rgb}{0.58,0,0.82}      % 紫色 (用于 listings 字符串)
\definecolor{backcolour}{rgb}{0.95,0.95,0.92}    % 浅背景色 (可用于自定义代码块背景，当前 listings 未直接使用)

%---------------------------- 代码块格式设置 (listings) ----------------------------%
\counterwithin{figure}{section} % 图表编号与章节关联 (此行通常用于图表，与listings直接关系不大，但常放在类似配置区)
\lstset{
  commentstyle= \color{red!50!green!50!blue!50},  % 注释样式：一种混合灰色
  ...
  captionpos=b,                                   % 代码块标题位置 (b: 底部, t: 顶部)
...
%--------------------------------- 页面边距设置 (geometry) -------------------------------------%
\geometry{left=2.5cm,right=2.5cm,top=2.5cm,bottom=2.5cm} % 设置A4纸的上下左右页边距
%--------------------------------- 页面边距设置完成 -------------------------------------%

%-------------------------- 页眉页脚样式定义 (fancyhdr) -------------------------------%
\setlength{\headheight}{15pt} % 设置页眉高度，以容纳页眉内容

\fancypagestyle{plain}{ % 定义 "plain" 页面样式 (通常用于章节首页、空白页)
  \fancyhf{} % 清空所有页眉页脚域
...
\setlength{\itemsep}{1.5pt} % 调整各参考文献条目之间的垂直间距
  ...
  \bibitem[5]{ref5}马驰, 张国群, 孙俊格, 等. 基于深度强化学习的综合电子系统重构方法[J]. 空天防御, 2024, 7(1): 63-70.
  \bibitem[6]{ref6}马驰, 张国群, 孙俊格, 等. 基于深度强化学习的综合电子系统重构方法[J]. 空天防御, 2024, 7(1): 63-70.
  \bibitem[7]{ref7}马驰, 张国群, 孙俊格, 等. 基于深度强化学习的综合电子系统重构方法[J]. 空天防御, 2024, 7(1): 63-70.
  \bibitem[8]{ref8}马驰, 张国群, 孙俊格, 等. 基于深度强化学习的综合电子系统重构方法[J]. 空天防御, 2024, 7(1): 63-70.
\end{thebibliography}
